...
def search_cves(service, version):
    ...
    # Fonction de requête avec réessai
    def make_request():
        try:
    ...
    # Essayer jusqu'à 3 fois avec des délais progressifs
    for attempt in range(3):
        if make_request():
            break
        print(f"Retrying... ({attempt + 1}/3)")
        time.sleep(2 ** attempt)  # Delays of 1, 2, 4 seconds
    else:
        print(f"Failed to fetch CVEs for {query} after 3 attempts")
...
def perform_dictionary_attack(host, auth_type):
    ...
    try:
        print(f"Lancement de Hydra pour {auth_type} avec les fichiers {user_dictionary_file} et {password_dictionary_file}")
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()
        
        # Afficher les sorties de Hydra
        print(f"Hydra output:\n{stdout.decode()}")
        print(f"Hydra errors:\n{stderr.decode()}")

        if "login:" in stdout.decode():
            print(f"Connexion réussie: {stdout.decode()}")
            for line in stdout.decode().splitlines():
                if "login:" in line:
                    ...
                    hydra_results.append((username, password, timestamp, auth_type))
            return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    scan_results = scan_services_and_versions(host)
    
    # Search for CVEs
    for host_info in scan_results.values():
        for protocol, services in host_info['protocols'].items():
            for service in services:
                cves = search_cves(service['service'], service['version'])
                service['cves'] = cves
    
    # Perform dictionary attacks
    if perform_dictionary_attack(host, 'FTP'):
        print("FTP dictionary attack results will be included in the report")
    if perform_dictionary_attack(host, 'SSH'):
        print("SSH dictionary attack results will be included in the report")
    
    # Generate PDF report
    generate_pdf_report("Security_Report", scan_results)
